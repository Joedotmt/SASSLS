...
<div class="search-area panel {env.minimized ? 'minimized' : ''}">
    ...
    <div style="padding: 1em {env.minimized ? 5 : 1}em; overflow-y: auto; overflow-x:hidden; transition: padding 0.2s cubic-bezier(0.4, 0, 0, 1); gap:0.3em; display: flex; flex-direction: column;" inert={env.minimized ? "inert" : ""}>
        ...
        <Accordion overflowy="hidden">
            ...
            {#snippet details()}
                <div style="flex-direction: row; display:flex; justify-content: space-between;">
                    <ChipGroup bind:selected={env.searchState.sortType} multiple={false} optional={false} defaultIndex={0} style="width: fit-content" items={["Created", "Updated"]} />
        ...
        <Accordion overflowy="hidden">
            ...
            {#snippet details()}
                <div>
                    <ChipGroup bind:selected={env.searchState.group} items={constants.borrowers.groups} />
                </div>
            {/snippet}
        </Accordion>
    </div>
